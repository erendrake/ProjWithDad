...
{
  Serial.println("Process Tank Pump ");
  // minTankPumpOnSeconds = 120
  // const int systemDiffOn = SYSTEM_DIFF_ON;             // 8°
  // const int systemDiffOff = SYSTEM_DIFF_OFF;           // 2°
  // const int systemOverheat = SYSTEM_OVERHEAT;          // Tank Temp >= 140°
  systemDifference = (currentPanelTemp - currentTankTemp);
  Serial.print("Current Panel Temp ");
  Serial.print(currentPanelTemp, 1);
  Serial.print(" - Current Tank Temp ");
  Serial.print(currentTankTemp, 1);
  Serial.print(" = ");
  ...
  {
    pumpOnTime = millis();                            // Start the 2 min Timer
    tankPump = true;
    pumpOn();                                           // Turn on the Pump

  }
  //msg_tank_pump(Tank_Pump_ID, V_STATUS);
  ...
  Serial.print ("Current Pump On Time= ");
  Serial.print((currentPumpOnTime / 600000), 1);
  Serial.println (" Min.");
  if (currentPumpOnTime >= minPumpOnTime)
  {
    // If the 2 Min Clock Has Run Out and the Solar Panel Temp is <= to the Tank Temp Turn Off the Pump
    if (systemDifference <= systemDiffOff)
    {
      pumpOff();
    }
  }
...
void pumpOn()
{
  ...
  tankPump = true;
  send(msg_tank_pump.setSensor(Tank_Pump_ID).set(1));                   //  Send GW New Status
}
...
void pumpOff()
{
  ...
  tankPump = false;
  send(msg_tank_pump.setSensor(Tank_Pump_ID).set(0));                   //  Send GW New Status}
}
  ...
  {
    Serial.println("readTankPumpPressure");
  ...
  {
    Serial.println("readTankPumpFlow.");
